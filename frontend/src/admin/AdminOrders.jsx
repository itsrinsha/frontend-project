...
const AdminOrderSection = () => {
  ...
  const fetchOrders = async (page = 1, search = "") => {
    try {
      const res = await api.get(`/admin/orders?page=${page}&limit=10&search=${search}`);
      const { orders: fetchedOrders, pagination } = res.data;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
          {/* Table Header */}
          <div className="bg-gradient-to-r from-gray-800 to-gray-700 px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
            <h2 className="text-xl font-semibold text-white">All Orders ({totalOrders})</h2>
            <form onSubmit={handleSearch} className="flex w-full md:w-auto gap-2">
              <input
                ...
                className="px-4 py-2 rounded-lg bg-gray-700 text-white border border-gray-600 focus:outline-none focus:border-blue-500 w-full md:w-64"
              />
              <button
                type="submit"
                              ...
                              {order.items?.map((item) => (
                                ...
                                >
                                  ...
                                  <div className="flex-1">
                                    <p className="font-semibold text-gray-800 mb-1 line-clamp-2">{item.name || item.title}</p>
        ...
        {orders.length > 0 && (
          <div className="mt-4 text-center text-sm text-gray-500">
            Showing {orders.length} orders on page {currentPage} of {totalPages}
          </div>
        )}
      </div>
    </div>
